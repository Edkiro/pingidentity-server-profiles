...
if [[ ! -z "${OPERATIONAL_MODE}" && "${OPERATIONAL_MODE}" = "CLUSTERED_ENGINE" ]]; then
    ...
    while true; do
    curl -ss --silent -o /dev/null -k https://${pahost}:${PA_ADMIN_PUBLIC_PORT}/pa/heartbeat.ping 
    if ! test $? -eq 0 ; then
    ...
    echo "https://${pahost}:${PA_ADMIN_PUBLIC_PORT}/pa-admin-api/v3/httpsListeners"
    ...
    kpalias=$( curl -v -k -u Administrator:"${INITIAL_ADMIN_PASSWORD}" -H "X-Xsrf-Header: PingAccess" https://${pahost}:${PA_ADMIN_PUBLIC_PORT}/pa-admin-api/v3/keyPairs | jq '.items[] | select(.id=='${keypairid}') | .alias' )
    echo "Key Pair Alias:"${kpalias}

    echo "Retrieving Engine Certificate ID..."
    certid=$( curl -v -k -u Administrator:"${INITIAL_ADMIN_PASSWORD}" -H "X-Xsrf-Header: PingAccess" https://${pahost}:${PA_ADMIN_PUBLIC_PORT}/pa-admin-api/v3/engines/certificates| jq '.items[] | select(.alias=='${kpalias}' and .keyPair==true) | .id' )
    ...
    engineid=$( curl -v -k -X POST -u Administrator:"${INITIAL_ADMIN_PASSWORD}" -H "X-Xsrf-Header: PingAccess" -d "{
        ...
        }" https://${pahost}:${PA_ADMIN_PUBLIC_PORT}/pa-admin-api/v3/engines | jq '.id' )

    echo "EngineId:"${engineid}
    set PA_ENGINE_ID=${engineid}
    echo "Retrieving the engine config..."
    curl -v -k -X POST -u Administrator:"${INITIAL_ADMIN_PASSWORD}" -H "X-Xsrf-Header: PingAccess" https://${pahost}:${PA_ADMIN_PUBLIC_PORT}/pa-admin-api/v3/engines/${engineid}/config -o engine-config.zip
